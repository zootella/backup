...
void Report(read r) {
	...
	if (PROGRAMTEST) MessageBox(Handle.window, r, "Report", MB_OK);
}
...
void BeginThread(LPVOID function) {
	...
	if (!thread) Report("error beginthreadex");

	// Tell the system this thread is done with the new thread handle
	if (!CloseHandle(thread)) Report("error closehandle");
}
...
void WindowTextSet(HWND window, read r) {
	...
	if (!SetWindowText(window, r)) Report("error setwindowtext");
}
...
HWND WindowCreateEdit(bool scrollbars, bool capacity) {
	...
	HWND window = WindowCreate("EDIT", NULL, style, 0, Handle.window, NULL);
...
HWND WindowCreateButton(read r) {
	...
	// Create the edit window
	HWND window = WindowCreate("BUTTON", NULL, style, 0, Handle.window, NULL);
...
HWND WindowCreate(read name, read text, DWORD style, int size, HWND parent, HMENU menu) {
	...
	HWND window = CreateWindow(
		...
		NULL);                  // No parameter
	if (!window) Report("error createwindow");
	return window;
}
...
void WindowSize(HWND window, sizeitem size) {
	...
	if (!MoveWindow(window, size.x(), size.y(), size.w(), size.h(), false)) Report("error movewindow");
}
...
string DialogBrowse(read display) {
	...
	if (!list) return ""; // The user clicked Cancel or the close X
