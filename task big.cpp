...
DWORD WINAPI Tasks() {
	...
	while (is(s) && JobContinue()) {
		...
		if (is(line)) {
			...
			string parse = line;
			split(parse, "\"", &task, &parse);
			split(parse, "\"", &source, &parse);
			split(parse, "\"", &parse, &destination);
			task = trim(task, " "); // Trim spaces
			source = trim(source, " ");
			destination = trim(destination, " ", "\""); // Trim spaces and remove trailing quote
			task = off(task, "\\", Reverse); // Remove trailing backslashes, "C:\" becomes "C:" so we can put on "\folder" later
			source = off(source, "\\", Reverse);
			destination = off(destination, "\\", Reverse);

			// Run the task
			if      (task == "Delete")         TaskDelete(source);
			else if (task == "Copy")           TaskCopy(source, destination);
			else if (task == "Compare")        TaskCompare(source, destination);
			else if (task == "Update")         TaskUpdate(source, destination, false);
			else if (task == "Update Compare") TaskUpdate(source, destination, true);
			else                               JobError("Cannot " + line); // Unknown task
		}
	}
...
void TaskDelete(read path) {
	...
	if (!DiskFolder(path, false, true)) { JobError(make("Cannot write \"", path, "\"")); return; }
...
void TaskCopy(read source, read destination) {
	...
	if (!DiskFolder(source, false, false)) { JobError(make("Cannot read \"", source, "\"")); return; }
	if (!DiskFolder(destination, true, true)) { JobError(make("Cannot write \"", destination, + "\"")); return; }
...
void TaskCompare(read source, read destination) {
	...
	if (!DiskFolder(source, false, false)) { JobError(make("Cannot read \"", source, "\"")); return; }
	if (!DiskFolder(destination, false, false)) { JobError(make("Cannot read \"", destination, "\"")); return; }
